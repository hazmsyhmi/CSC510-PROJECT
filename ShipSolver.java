...
public class ShipSolver {
    ...
    public boolean solve() {
        ...
        for (int[] nat : perms) {
            ...
            if (frenchPos >= greekPos) continue; // Skip this nationality arrangement
    ...
    public int getPortSaidPosition() {
        for (int i = 0; i < 5; i++) {
            if (solDest[i] == ShipData.PORT_SAID) return i;
        }
        return -1;
    }
    ...
    public int getTeaPosition() {
        for (int i = 0; i < 5; i++) {
            if (solCargo[i] == ShipData.TEA) return i;
        }
        return -1;
    }
